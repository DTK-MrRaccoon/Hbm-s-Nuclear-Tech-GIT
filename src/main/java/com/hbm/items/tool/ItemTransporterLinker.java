...
public class ItemTransporterLinker extends Item implements IGUIProvider {
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float fx, float fy, float fz) {
		...
		if(!(tile instanceof TileEntityTransporterBase)) return false;
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int i, boolean b) {
		...
		if(((EntityPlayerMP)entity).getHeldItem() != stack) return;
	...
	public static class TransporterInfo {
		...
		public void writeToNBT(NBTTagCompound nbt) { writeToNBT(nbt, true); }
		...
		public static TransporterInfo readFromNBT(NBTTagCompound nbt) {
			...
			if(nbt.hasKey("linked")) info.linkedTo = readFromNBT(nbt.getCompoundTag("linked"));
			return info;
		}

		@Override
		public int hashCode() { return Objects.hash(dimensionId, x, y, z); }

		@Override
		public boolean equals(Object other) {
			if(this == other) return true;
			if(!(other instanceof TransporterInfo)) return false;
			TransporterInfo info = (TransporterInfo) other;
			return dimensionId == info.dimensionId && x == info.x && y == info.y && z == info.z;
		}
	}
	...
	private List<TransporterInfo> getTransporters(ItemStack stack) {
		return new ArrayList<>(loadTransporters(stack));
	}
	...
	private static void saveTransporters(ItemStack stack, Set<TransporterInfo> transporters) {
		...
		for(TransporterInfo info : transporters) {
			if(!data.containsKey(info.dimensionId)) data.put(info.dimensionId, new ArrayList<>());
