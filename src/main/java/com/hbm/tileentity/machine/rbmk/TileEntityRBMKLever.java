...
public class TileEntityRBMKLever extends TileEntityLoadedBase implements IGUIProvider, IControlReceiver, SimpleComponent, CompatHandler.OCComponent {
	...
	public class LeverUnit {
		...
		public void click() {
			...
			if(this.flipProgress <= 0 || this.flipProgress >= 1)
				worldObj.playSoundEffect(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, "hbm:block.leverStart", 1F, 1F);
		...
		public void update() {
			...
			if(this.isTurningOn && this.flipProgress < 1F) {
				...
				if(this.flipProgress >= 1F) {
					...
					worldObj.playSoundEffect(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, "hbm:block.leverStop", 0.5F, 1F);
					arcFlash = true;
				}
			...
			if(arcFlash) {
				...
				worldObj.playSoundEffect(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, "hbm:block.spark", 1F, 1F);
	...
	// OpenComputers methods - READ ONLY for state, can set label/active
	@Override
