...
public class ContainerMachineMixingVat extends Container {

	private TileEntityMachineMixingVat mixingVat;

	public ContainerMachineMixingVat(InventoryPlayer invPlayer, TileEntityMachineMixingVat tedf) {
		mixingVat = tedf;
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int index) {
		...
		if(slot != null && slot.getHasStack()) {
			...
			if(index <= 20) {
				...
				} else if(rStack.getItem() instanceof ItemMixingVatTemplate) {
					if(!this.mergeItemStack(stack, 4, 5, false)) return null;
				} else {
					// Try to put in input slots first, then fluid input slots
					if(!this.mergeItemStack(stack, 13, 17, false))
					if(!this.mergeItemStack(stack, 17, 19, false))
						return null;
				}
			}
	...
	public boolean canInteractWith(EntityPlayer player) {
		return mixingVat.isUseableByPlayer(player);
	}
}