...
public class GUITransporterLinker extends GuiScreen {
	...
	protected void drawGuiContainerBackgroundLayer(int mouseX, int mouseY) {
		...
		// textLeftX adjusted from 62 to 37 to occupy icon space
		int textLeftX = guiLeft + 37;
		int textRightX = guiLeft + 188;

		// Draw current transporter text (Top Section)
		String coordinates = "x: " + linkFromTransporter.x + ", z: " + linkFromTransporter.z;
		int coordWidth = fontRendererObj.getStringWidth(coordinates);
		
		fontRendererObj.drawStringWithShadow(linkFromTransporter.name, textLeftX, guiTop + 13, 0x00ff00);
		fontRendererObj.drawStringWithShadow(coordinates, textRightX - coordWidth, guiTop + 23, 0x00ff00);
		
		// Draw linkable transporters list
		for(int i = index; i < Math.min(index + 5, visibleTransporters.size()); i++) {
			...
			coordWidth = fontRendererObj.getStringWidth(coordinates);
			
			fontRendererObj.drawStringWithShadow(transporter.name, textLeftX, y - 1, 0x00ff00);
			fontRendererObj.drawStringWithShadow(coordinates, textRightX - coordWidth, y + 9, 0x00ff00);
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if(visibleTransporters.size() > 5) {
			int sx = guiLeft + 202;
			int sy = guiTop + 68 + MathHelper.floor_float((float)index / (float)(visibleTransporters.size() - 5) * 114.0F);
			drawTexturedModalRect(sx, sy, xSize, 54, 14, 15);
		}
	}
